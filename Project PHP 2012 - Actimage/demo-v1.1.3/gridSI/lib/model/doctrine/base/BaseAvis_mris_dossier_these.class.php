<?php

/**
 * BaseAvis_mris_dossier_these
 * 
 * This class has been auto-generated by the Doctrine ORM Framework
 * 
 * @property integer $avis_mris_id
 * @property integer $dossier_these_id
 * @property Avis_mris $Avis_mris
 * @property Dossier_these $Dossier_these
 * 
 * @method integer                 getAvisMrisId()       Returns the current record's "avis_mris_id" value
 * @method integer                 getDossierTheseId()   Returns the current record's "dossier_these_id" value
 * @method Avis_mris               getAvisMris()         Returns the current record's "Avis_mris" value
 * @method Dossier_these           getDossierThese()     Returns the current record's "Dossier_these" value
 * @method Avis_mris_dossier_these setAvisMrisId()       Sets the current record's "avis_mris_id" value
 * @method Avis_mris_dossier_these setDossierTheseId()   Sets the current record's "dossier_these_id" value
 * @method Avis_mris_dossier_these setAvisMris()         Sets the current record's "Avis_mris" value
 * @method Avis_mris_dossier_these setDossierThese()     Sets the current record's "Dossier_these" value
 * 
 * @package    gridSI
 * @subpackage model
 * @author     Your name here
 * @version    SVN: $Id: Builder.php 7691 2011-02-04 15:43:29Z jwage $
 */
abstract class BaseAvis_mris_dossier_these extends gridDoctrine_Record
{
    public function setTableDefinition()
    {
        $this->setTableName('avis_mris_dossier_these');
        $this->hasColumn('avis_mris_id', 'integer', null, array(
             'type' => 'integer',
             'primary' => true,
             ));
        $this->hasColumn('dossier_these_id', 'integer', null, array(
             'type' => 'integer',
             'primary' => true,
             ));
    }

    public function setUp()
    {
        parent::setUp();
        $this->hasOne('Avis_mris', array(
             'local' => 'avis_mris_id',
             'foreign' => 'id',
             'onDelete' => 'CASCADE'));

        $this->hasOne('Dossier_these', array(
             'local' => 'dossier_these_id',
             'foreign' => 'id',
             'onDelete' => 'CASCADE'));
    }
}