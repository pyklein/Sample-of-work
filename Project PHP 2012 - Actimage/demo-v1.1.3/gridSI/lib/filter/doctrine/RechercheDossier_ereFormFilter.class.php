...
/**
 * Filtre pour la recherche de dossier ERE
 *
 * @author Alexandre WETTA
 */
class RechercheDossier_ereFormFilter extends BaseDossier_ereFormFilter {
  ...
  public function configure() {
    ...
    $this->setWidget('laboratoire', new sfWidgetFormDoctrineChoice(
                    array('model' => 'Dossier_ere_laboratoire',
  ...
  protected function addLaboratoireColumnQuery($query, $field, $value) {
    Dossier_ereTable::getInstance()->appliquerFiltreLaboratoire($query, $value);
  }

  protected function addEtudiantColumnQuery($query, $field, $value) {
    if ($value['text'] != '') {
      Dossier_ereTable::getInstance()->appliquerFiltreEtudiant($query, $value['text']);
    }
  }
  ...
  protected function addTitreColumnQuery($query, $fiel, $value) {
    Dossier_ereTable::getInstance()->appliquerFiltreTitre($query, $value['text'], $this->getValue("etou_titre"));
  }

  protected function addEncadrantColumnQuery($query, $field, $value) {
    if ($value['text'] != '') {
      Dossier_ereTable::getInstance()->appliquerFiltreEncadrant($query, $value['text']);
    }
  }
