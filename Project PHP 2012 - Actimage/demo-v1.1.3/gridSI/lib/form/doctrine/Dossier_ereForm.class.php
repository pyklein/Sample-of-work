 ...
 */
class Dossier_ereForm extends BaseDossier_ereForm
{
  protected $utilArbo;
  ...
  public function configure()
  {
    ...
    $this->configurerWidgets();
    $this->disableLocalCSRFProtection();
    parent::configure();
  }

  private function configurerWidgets()
  {
  ...
  private function configurerValidateurs()
  {
    ...
    $this->validatorSchema['fichier_pdf'] = new gridValidatorFile(
            array('max_size' => $utilPhp->getMaxUploadSizeEnBytes(),
                  'fichier_pdf' => true,
                  'path' => $this->utilArbo->getRepertoireDossiersEre(),
                  'required' => false
                  ),
   ...
   public function  save($con = null) {
    ...
    if($this->getObject()->getNumeroAAfficher() != $this->values['numero_provisoire'] &&
      ...
      if($this->values['numero_definitif'] != null){
        $strDestination = $utilArbo->getRepertoireConventionDossierEre($this->values['numero_definitif']) ;
      }else if($this->values['numero_provisoire']!= null){
        $strDestination = $utilArbo->getRepertoireConventionDossierEre($this->values['numero_provisoire']) ;
      }

      //on crée le nouveau répertoire
      $utilFichier = new UtilFichier();
      $utilFichier->moveRepertoire($utilArbo->getRepertoireConventionDossierEre($this->getObject()->getNumeroAAfficher()), $strDestination) ;
