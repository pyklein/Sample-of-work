...
<div class="right">
  <?php echo link_to_grid(libelle("msg_bouton_ajouter_statut"), "referentiel_bpi/creerStatut_Dossier_bpi",array("class" =>"picto bt_ajouter")); ?>
</div>
  ...
  <table class="mep">
    ...
    <?php foreach ($objStatuts as $intCle => $objStatut): ?>
      <tr class="<?php echo $intCle%2 == 0 ? "pair" : "impair" ?>">
        <td>
          <?php echo link_to("","referentiel_bpi/modifierStatut_Dossier_bpi?id=".$objStatut->getId(), array("class" => "picto_court bt_modifier","title" => libelle("msg_bouton_modifier"))); ?>
          <?php
          echo $objStatut->getEstActif() ?
                  link_to_grid("","referentiel_bpi/changerActivationStatutDossierBpi?id=".$objStatut->getId(), array("class" => "picto_court bt_desactiver","title"=> libelle("msg_bouton_desactiver"))) :
                  link_to_grid("","referentiel_bpi/changerActivationStatutDossierBpi?id=".$objStatut->getId(), array("class" => "picto_court bt_activer","title" => libelle("msg_bouton_activer")))
          ?>
        </td>
        <td>
