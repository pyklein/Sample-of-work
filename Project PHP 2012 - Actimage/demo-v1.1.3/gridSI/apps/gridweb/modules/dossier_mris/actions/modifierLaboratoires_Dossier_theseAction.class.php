...
/**
 * Description of modifierLaboratoires_Dossier_these
 *
 * @author Simeon Petev
 */
class modifierLaboratoires_Dossier_theseAction extends gridAction
{
  public function  preExecute()
  {
    ...
    if (($this->objDossier == null) || ($this->objDossier->getId()==0))
    {
      if ($idDossier != null)
      {
        $this->getUser()->setFlash("erreur", libelle('msg_dossier_these_droit_laboratoires'));
      }
  ...
  public function  execute($request)
  {
    ...
    if (!$srvToken->hasToken('modifier_laboratoires_dossier_these'))
    {
      $strToken = $srvToken->creerToken('modifier_laboratoires_dossier_these', $this->objDossier->getId());
    } else
    {
      $strToken = $srvToken->getToken('modifier_laboratoires_dossier_these');
    }
    ...
    if ($request->isMethod('get'))
    {
      ...
      } else if ($request->hasParameter('start'))
      {
        ...
        $this->getUser()->setAttribute('filtre_labo_dossier_these', array('organisme_id'=>"","service_id"=>""));
      }

      if (!$this->getUser()->hasAttribute('filtre_labo_dossier_these'))
      {
        $this->getUser()->setAttribute('filtre_labo_dossier_these', array('organisme_id'=>"","service_id"=>""));
      }
    } 
    ...
    elseif ($request->isMethod('post') && $request->hasParameter("chargerVilles"))
    {
    ...
    else
    {
      ...
      {
        $this->getUser()->offsetUnset('filtre_labo_dossier_these');

        if ($request->hasParameter('labo_filtre_submit'))
        {
          $this->getUser()->setAttribute('filtre_labo_dossier_these', $request->getParameter($this->objFiltreLaboratoires->getName()));
        } else
        {
          $this->getUser()->setAttribute('filtre_labo_dossier_these', array('organisme_id'=>"","service_id"=>""));
        }
    ...
    $this->objFiltreLaboratoires->bind($this->getUser()->getAttribute('filtre_labo_dossier_these'));
    ...
    $objQueryLaboratoiresDispo = LaboratoireTable::getInstance()->buildQueryLaboratoiresDisponiblesAvecFiltreSauf($this->objFiltreLaboratoires, Session_dossier_these_laboratoireTable::getInstance()->getIdsLaboratoiresAssocies($strToken,$this->objDossier->getId()));
  ...
  private function getLaboratoiresAssocies($strToken,$boolABlank=false)
  {
    ...
    $arrLaboratoiresTmp = Session_dossier_these_laboratoireTable::getInstance()->getQueryObject()->where('transaction_token = ?',$strToken)->execute();

    if ($boolABlank)
    {
      //Efface des valeurs eventuelles
      try {
        Session_dossier_these_laboratoireTable::getInstance()->getQueryObject()->where('transaction_token = ?',$strToken)->delete()->execute();
      } catch (Exception $exc) {
      ...
      try {
        ...
        {
          $objLaboratoireTmp = new Session_dossier_these_laboratoire();

          $objLaboratoireTmp->setDossierThese($this->objDossier);
  ...
  private function ajouterLaboratoireDispo($intIdLaboratoire,$strToken)
  {
    ...
    $objLaboratoire = new Session_dossier_these_laboratoire();

    $objLaboratoire->setDossierThese($this->objDossier);
    ...
    $arrListeLaboratoiresTemp = Session_dossier_these_laboratoireTable::getInstance()->getQueryObject()->where('transaction_token = ?',$strToken)->execute();
  ...
  private function retirerLaboratoire($intArrayOffset,$strToken)
  {
    ...
    try {
      ...
      Session_dossier_these_laboratoireTable::getInstance()->getQueryObject()->where('id = ?',$intArrayOffset)->andWhere('transaction_token = ?',$strToken)->delete()->execute();
  ...
  private function enregistrer($strToken)
  {
    ...
    try {
      $arrLabos = LaboratoireTable::getInstance()->retreveLaboratoiresAvecIds(Session_dossier_these_laboratoireTable::getInstance()->getIdsLaboratoiresAssocies($strToken,$this->objDossier->getId()));
