...
/**
 * Statistiques et Tableau de bord des dossiers de stage postdoctoraux
 *
 * @author Jihad SAHEBDIN
 */
class voirRapportStatistiquesEtTableauDeBord_Dossier_postdocAction extends gridAction
{
  public function execute($request) 
  {
    ...
    $this->intNbDossiersProposition = Dossier_postdocTable::getInstance()->findAll()->count();
    ...
    $utilFichier->purgerFichiers($utilArbo->getRepertoireTemporaire(), "(stat_dossier_postdoc).*(\.png)", 60);
...
//      Répartition par domaines scientifique
      if ($this->groupPropositionDS)
      {
        ...
        $libelle = array() ;
        $libelle2 = array() ;
        foreach (Dossier_postdocTable::getInstance()->getCountsByDomaineScientifique($objRequeteDoctrine) as $objDomaineScientifique => $compte) {
          if ($compte > 0) {
            $donnees[] = $compte;
        ...
        if (count($donnees) > 0) {
          $this->chartPropositionDS = $srvChart->creerCamembertLegendeACote($libelle, $donnees, $libelle2, "stat_dossier_postdoc_proposition_dom_scientifique");
        }
      }
      ...
      if ($this->groupDossierDS)
      {
        ...
        $result = $result->andWhere('statut_dossier_postdoc_id = ?',  Statut_dossier_postdocTable::VALIDE);
        ...
        $libelle = array() ;
        $libelle2 = array() ;
        foreach (Dossier_postdocTable::getInstance()->getCountsByDomaineScientifique($result) as $objDomaineScientifique => $compte) {
          if ($compte > 0) {
            $donnees[] = $compte;
            $libelle[] = libelle_graphiques("msg_statistiques_bpi_libelle_dossier", array($compte));
            $libelle2[] = $objDomaineScientifique;
        ...
        //Création du graphique
        if (count($donnees) > 0) {
          $this->chartDossierDS = $srvChart->creerCamembertLegendeACote($libelle, $donnees, $libelle2, "stat_dossier_postdoc_dossier_dom_scientifique");
        }
      }
      ...
      {
        $result = clone $objRequeteDoctrine;
        $result = $result->andWhereIn('statut_dossier_postdoc_id',array(Statut_dossier_postdocTable::PROPOSITION,Statut_dossier_postdocTable::REFUSE, Statut_dossier_postdocTable::MIS_EN_ATTENTE ));

        $valide = clone $objRequeteDoctrine;
        $valide = $valide->andWhere('statut_dossier_postdoc_id = ?',  Statut_dossier_postdocTable::VALIDE);
        ...
        $maxVal = 0;
        $maxVal2 = 0;
        foreach (Dossier_postdocTable::getInstance()->getCountsByRegion($result) as $region => $compte)
        {
          $donnees[] = $compte;
          $libelle[] = $region;
        ...
        foreach (Dossier_postdocTable::getInstance()->getCountsByRegion($valide) as $region => $compte2)
        {
          $donnees2[] = $compte2;
        }
        ...
        if (count($donnees) > 0) {
          ...
          $srvChart->genererGraphique("stat_dossier_postdoc_region");
...
//      Groupements répartition par origine
      if ($this->groupPropositionOrigine)
      {
        ...
        $libelle = array() ;
        $libelle2 = array() ;
        foreach (Dossier_postdocTable::getInstance()->getCountsByOrigine($objRequeteDoctrine) as $objCursus => $compte) {
          if ($compte > 0) {
            $donnees[] = $compte;
            $libelle[] = libelle_graphiques("msg_statistiques_bpi_libelle_dossier", array($compte));
            $libelle2[] = $objCursus;
        ...
        //Création du graphique
        if (count($donnees) > 0) {
          $this->chartPropositionOrigine = $srvChart->creerCamembert($libelle, $donnees, $libelle2, "stat_dossier_postdoc_proposition_origine");
        }
      }
      ...
      if ($this->groupDossierOrigine)
      {
        ...
        $libelle = array() ;
        $libelle2 = array() ;
        foreach (Dossier_postdocTable::getInstance()->getCountsByOrigine($result) as $objCursus => $compte) {
          if ($compte > 0) {
            $donnees[] = $compte;
            $libelle[] = libelle_graphiques("msg_statistiques_bpi_libelle_dossier", array($compte));
            $libelle2[] = $objCursus;
        ...
        //Création du graphique
        if (count($donnees) > 0) {
          $this->chartDossierOrigine = $srvChart->creerCamembert($libelle, $donnees, $libelle2, "stat_dossier_postdoc_dossier_origine");
        }
      }
    }
...
//    Nombre de dossiers par organisme
    ...
    {
      $intIndex = 0;
      foreach (Dossier_postdocTable::getInstance()->getCountsByOrganisme($objRequeteDoctrine) as $org => $compte)
      {
      ...
      $result = $result->andWhere('statut_dossier_postdoc_id = ?',  Statut_dossier_postdocTable::VALIDE);
      $intIndex = 0;
      foreach (Dossier_postdocTable::getInstance()->getCountsByOrganisme($result) as $org => $compte)
      {
