...
<div id="zone_cadre" class="reduit">
  ...
  <?php if($objPager->count() != 0): ?>
    <fieldset>
      <legend>
        <?php echo libelle("msg_fieldset_suivi_ere")?>
      </legend>
      ...
      <?php foreach ($objPager->getResults() as $objSuivi): ?>
        ...
        <p><?php echo $objSuivi->getDescriptif()." - ".
                link_to_grid(libelle("msg_suivi_ere_libelle_edit"),"dossier_mris/modifierSuivi". $strModelContenant."?id=".$objSuivi->getId()."&dossier_id=".$objDossier->getId(),array("class" => "picto bt_modifier"))." ".
                link_to_grid(libelle("msg_suivi_ere_libelle_del"),"dossier_mris/supprimerSuivi". $strModelContenant."?id=".$objSuivi->getId()."&dossier_id=".$objDossier->getId(),array("class" => "picto bt_supprimer")) ?>
        </p>
        <p><?php if($objSuivi->getDate_reception() != null)
                 {
                    echo libelle("msg_message_suivi_element_recu_ere",array(formatDate($objSuivi->getDate_demande()),formatDate($objSuivi->getDate_reception()),formatDate($objSuivi->getDate_echeance())));
                 }
                 else
                 {
                    echo libelle("msg_message_suivi_element_nonrecu_ere",array(formatDate($objSuivi->getDate_demande()),formatDate($objSuivi->getDate_echeance())));
                 }
            ?>
        </p>
       <br>
