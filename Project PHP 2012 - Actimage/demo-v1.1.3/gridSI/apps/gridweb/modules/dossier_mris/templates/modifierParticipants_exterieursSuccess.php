...
<?php include_partial('dossier_mris/gestion_commissions', array('strId' => $strId, 'ongletActif' => 3)) ?>
...
<div>
  <div id="zone_cadre">
    ...
    <fieldset>
      <legend><?php echo libelle('msg_participants_exterieurs_libelle_disponibles') ?></legend>
      <?php if ($objPager1->count() == 0) : ?>
        <?php echo libelle('msg_participants_exterieurs_0_disponible') ?>
      <?php else : ?>
        <table class="mep">
          <thead>
            <tr>
              ...
              <th width="20%"><?php echo libelle("msg_utilisateur_libelle_prenom") ?></th>
              <th width="25%"><?php echo libelle("msg_libelle_fonction_titre") ?></th>
              <th width="30%"><?php echo libelle("msg_libelle_organisme") ?></th>
            </tr>
          </thead>
          <tbody>
          <?php foreach ($objPager1->getResults() as $clef => $utilisateur): ?>
            <tr class="<?php echo $clef % 2 == 0 ? "pair" : "impair" ?>">
              <td>
                <?php echo link_to("", "dossier_mris/ajouterParticipant_exterieurs?commission=" . $strId . "&participant=" . $utilisateur->getId(), array("class" => "picto_court bt_ajouter", "title" => libelle("msg_bouton_ajouter"))); ?>
              </td>
              <td><?php echo $utilisateur->getNom() ?></td>
              <td><?php echo $utilisateur->getPrenom() ?></td>
              <td><?php echo $utilisateur->getTitre() ?></td>
              <td><?php echo $utilisateur->getOrganisme() ?></td>
            </tr>
          <?php endforeach; ?>
          </tbody>
        </table>
    ...
    <fieldset>
      <legend><?php echo libelle('msg_participants_exterieurs_libelle_concernes') ?></legend>
      <?php if ($objPager2->count() == 0): ?>
        <?php echo libelle('msg_participants_exterieurs_0_concernes') ?>
      <?php else : ?>
        <table class="mep">
          <thead>
            <tr>
          ...
          <?php foreach ($objPager2 as $clef => $utilisateur): ?>
            <tr class="<?php echo $clef % 2 == 0 ? "pair" : "impair" ?>">
              <td>
                <?php echo link_to("", "dossier_mris/retirerParticipant_exterieurs?commission=" . $strId . "&participant=" . $utilisateur->getId(), array("class" => "picto_court bt_supprimer", "title" => libelle("msg_bouton_retirer"))); ?>
              </td>
              <td><?php echo $utilisateur->getNom() ?></td>
              <td><?php echo $utilisateur->getPrenom() ?></td>
              <td><?php echo $utilisateur->getTitre() ?></td>
