...
<?php echo 'Le ' . date("d/m/y") . ' à ' . date("H:i:s"); ?><br>
  <?php if ($editPDF) : ?>
    ...
    <?php if ($isProposition && $hasCredentialsEvaluation) { ?>
        ...
        <?php include_component("dossier_mris", "voirEvaluationDossier_these", array("id" => $strId)); ?>
    <?php } ?>

    <!-- ONGLET SUIVI / ABOUTISSEMENT -->
    <div><big><?php echo libelle("msg_libelle_suivi_aboutissement_dossier"); ?></big></div>
    <?php include_component("dossier_mris", "voirSuiviDossier_these", array("id" => $strId)); ?>

  <?php else : ?>
    <div><big><?php echo libelle("msg_dossier_these_aucun_resultat"); ?></big></div>
  <?php endif; ?>

<?php echo pdf_fin("dossier_these_".$strId.".pdf", $objDossierThese->getTitre()); ?>
