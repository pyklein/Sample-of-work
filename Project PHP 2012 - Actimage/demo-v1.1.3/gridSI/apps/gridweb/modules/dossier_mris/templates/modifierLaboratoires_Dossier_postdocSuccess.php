...
<?php include_partial('dossier_mris/onglet_dossier', array('strNomModel' => 'Dossier_postdoc', 'strId' => $objDossier->getId(), 'ongletActif' => 4)) ?>

<div id="zone_cadre" class="reduit">
  <div class="boutons">
    <form action="<?php echo url_for('dossier_mris/modifierLaboratoires_Dossier_postdoc')?>" method="post">
          ...
          <tbody>
            ...
            <?php foreach ($arrLaboratoiresAssocies as $clef => $laboratoireAssoc): ?>
              <tr class="<?php echo $i%2 == 0 ? "pair" : "impair" ?>">
                <td>
                  <?php echo link_to("", 'dossier_mris/modifierLaboratoires_Dossier_postdoc?dossier_postdoc_id='.$objDossier->getId().'&laboratoire_associe='.$laboratoireAssoc->getId(), array("class" => "picto_court bt_supprimer", "title" => libelle("msg_bouton_retirer"))); ?>
                </td>
  ...
  <form action="<?php echo url_for('dossier_mris/modifierLaboratoires_Dossier_postdoc?dossier_postdoc_id='.$objDossier->getId()) ?>" method="post" >
