...
<?php include_partial('dossier_mris/onglet_dossier', array('strNomModel' => 'Dossier_these', 'strId' => $objDossier->getId(), 'ongletActif' => 3)) ?>

<div id="zone_cadre" class="reduit">
  <div class="boutons">
    <form action="<?php echo url_for('dossier_mris/modifierEncadrants_Dossier_these')?>" method="post">
  ...
  <fieldset>
    <legend>
      <?php echo libelle('msg_dossier_these_fieldset_resp_dispo'); ?>
    </legend>

    <form action="<?php echo url_for('dossier_mris/modifierEncadrants_Dossier_these')?>" method="post">
      <?php echo $objFormEncadrantDossierThese; ?>
      <input type="hidden" name="dossier_these_id" value="<?php echo $objDossier->getId()?>"/>
  ...
  <fieldset>
    <legend>
      <?php echo libelle('msg_dossier_these_fieldset_encadrants'); ?>
    </legend>
          ...
          <tbody>
            ...
            <?php foreach ($arrEncadrantsAssocies as $clef => $encadrantAssoc): ?>
              <tr class="<?php echo $i%2 == 0 ? "pair" : "impair" ?>">
                <td>
                  <?php echo link_to("", 'dossier_mris/modifierEncadrants_Dossier_these?dossier_these_id='.$objDossier->getId().'&responsable_associe='.$encadrantAssoc->getId(), array("class" => "picto_court bt_supprimer", "title" => libelle("msg_bouton_retirer"))); ?>
                </td>
                ...
                <td><?php echo $encadrantAssoc->getRoleThese()->getIntitule() ?></td>
              </tr>
              <?php $i++; ?>
            <?php endforeach; ?>
          </tbody>
  ...
  <form action="<?php echo url_for('dossier_mris/modifierEncadrants_Dossier_these?dossier_these_id='.$objDossier->getId()) ?>" method="post" >
    <?php include_partial('referentiel_mris/intervenantForm', array('action' => 'ajout_intervenant', 'model' => 'Dossier_these', 'objForm' => $objFormAjoutIntervenant)) ?>
    <?php echo $objFormEncadrantDossierThese; ?>
